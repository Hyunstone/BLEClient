...
//        bleAdapter?.bluetoothLeScanner?.startScan(filters, settings, BLEScanCallback)
    ...
    private val gattClientCallback: BluetoothGattCallback = object : BluetoothGattCallback() {
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            ...
            sleep(1000)
            Log.d(TAG, "hi!" + status.toString())
            Log.d(TAG, "state!!" + newState.toString())
        ...
        override fun onServicesDiscovered(gatt: BluetoothGatt?, status: Int) {
            ...
            Log.d(TAG, "hhihihihi1")
            gatt.setCharacteristicNotification(respCharacteristic, true)

            Log.d(TAG, "characteristic")
            // 현재 여기서 막힘
            Log.d(TAG, respCharacteristic.toString())
//            Log.d(TAG, respCharacteristic.getStringValue())
//            Log.d(TAG, respCharacteristic.value.toString())
            readCharacteristic(respCharacteristic)

//            Log.d(TAG, "!!!descriptor!!!!")
//            for (descriptor in respCharacteristic.getDescriptors()) {
//                Log.e(TAG, "BluetoothGattDescriptor: " + descriptor.uuid.toString())
//            }
//
//            // UUID for notification
//            Log.d(TAG, "hhihihihi2")
//            val descriptor: BluetoothGattDescriptor = respCharacteristic.getDescriptor(
//                UUID.fromString(CLIENT_CHARACTERISTIC_CONFIG)
//            )
//            Log.d(TAG, "hhihihihi3")
//            descriptor.value = BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE
//            Log.d(TAG, "hhihihihi4")
//            gatt.writeDescriptor(descriptor)
    ...
    fun connectDevice(device: BluetoothDevice?) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            bleGatt = device?.connectGatt(MyApplication.applicationContext(), true, gattClientCallback, BluetoothDevice.TRANSPORT_LE)
        } else {
            bleGatt = device?.connectGatt(MyApplication.applicationContext(), true, gattClientCallback)
        }
    }
